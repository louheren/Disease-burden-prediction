...
data_temp=[]
for i in data:
    temp=list(i)[:2]+list(i)[3:]
    data_temp.append(temp)
data=data_temp
print(data)# train_X,test_X,train_y,test_y
train_X=[]
train_y=[]
temp = []
for line in data[:27]:
    temp.append(line)
    if len(temp)==3:
        train_X.append(temp[:2])
        train_y.append(temp[2][0])
        temp=[]
test_X=[]
test_y=[]
...
class LSTM(nn.Module):  # 注意Module首字母需要大写
    ...
    def forward(self,x):
        ...
        # print(lstm_out.shape)
        lstm_out=lstm_out.view(1,8)
        # print("lstm_out",lstm_out)
        lstm_out=self.linear_2(self.relu(self.linear_1(lstm_out)))
        return lstm_out
...
for epoch in range(epochs):
    ...
    for setp in range(len(train_X)):
        ...
        # print("120",        trainy = trainy.type(torch.FloatTensor)trainx,trainy)
        y_pred = model(trainx)
        # print("121",y_pred,trainy,type(y_pred),type(trainy))
...
with torch.no_grad():
    ...
    with torch.no_grad():
        ...
        for setp in range(len(test_X)):
            ...
            testy = testy.type(torch.FloatTensor)
            y_pred = model(testx)
            print(y_pred*7090,testy*7090)
            test_pred.append(y_pred[0][0].item()*7090)
            test_true.append(testy[0].item()*7090)
